...
var validator = require('jjv')({useDefault: true}); validator.addSchema(schema);
...
module.exports = function(config) {
	...
	function createAnswer(data) {
		...
		if(data.id)
			return Q.reject(new Error('The answer\'s ID must not be set.'));
	...
	function saveAnswer(id, data) {
		...
		if(data.id && id != data.id)
			return Q.reject(new Error('The answer\'s ID cannot be changed'));
