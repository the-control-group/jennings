...
function ArrDB(config, loadQuery, feedQuery){
	if (!(this instanceof ArrDB))
	  return new ArrDB(config);

	this.config = config;

	// optional queries allow for finer synchronization control
	this.loadQuery = loadQuery || r.db(this.config.rethinkdb.db).table(this.config.table);
	this.feedQuery = feedQuery || r.db(this.config.rethinkdb.db).table(this.config.table).changes();
...
ArrDB.prototype.query = function query(){
	...
	return Q.all([
		...
		// get entire database
		self.loadQuery.run(self.conn).then(function(cursor){
			return cursor.toArray().then(self.resetDb.bind(self));
		}),

		// begin changes stream
		self.feedQuery.run(self.conn)
