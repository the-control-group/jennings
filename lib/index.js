...
module.exports = function(config) {
	...
	function queryAnswers(clue, criteria) {
		...
		// get the entire db
		if(!clue && !criteria)
			return new Q(core.arrdb.db);

		// filter results
		var results = [];
		for (var i = 0; i < core.arrdb.db.length; i++) {

			// apply answer criteria to clue
			if(clue && core.arrdb.db[i].criteria && !core.filter(core.arrdb.db[i].criteria, clue)) continue;

			// apply query criteria to answers
			if(criteria && !core.filter(criteria, core.arrdb.db[i])) continue;

			results.push(core.arrdb.db[i]);
		}
